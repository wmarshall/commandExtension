 ...
 *
 * <p>
 * A {@link ConditionalCommand} adds the proper {@link Command} to the
 * {@link Scheduler} during {@link ConditionalCommand#initialize()} and then
 * {@link ConditionalCommand#isFinished()} will return true once that
...
public abstract class ConditionalCommand extends Command {
	 ...
	 *
	 * @param onTrue
	 *            The Command to execute if
	 *            {@link ConditionalCommand#condition()} returns true
	 */
	 ...
	 * @param onFalse
	 *            The Command to execute if
	 *            {@link ConditionalCommand#condition()} returns false
	 */
	public ConditionalCommand(Command onTrue, Command onFalse) {
		m_onTrue = onTrue;
		m_onFalse = onFalse;

		m_onTrue.getRequirementsStream().forEach(this::requires);
		m_onFalse.getRequirementsStream().forEach(this::requires);
	}

	/**
	 * Creates a new ConditionalCommand with given name and onTrue and onFalse
	 * Commands.
	 *
	 * <p>
	 * Users of this constructor should also override condition().
	 ...
	 *
	 * @return true if m_onTrue should be run or false if m_onFalse should be
	 *         run.
	 */
	...
	protected boolean isFinished() {
		return m_chosenCommand != null && m_chosenCommand.isRunning() && m_chosenCommand.isFinished();
	}
