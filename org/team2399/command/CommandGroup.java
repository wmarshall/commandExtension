...
public class CommandGroup extends Command {
	 ...
	 *
	 * @param command
	 *            The {@link Command Command} to be added
	 * @throws IllegalUseOfCommandException
	 *             if the group has been started before or been given to another
	 *             group
	 * @throws IllegalArgumentException
	 *             if command is null
	 */
	public final synchronized void addSequential(Command command) {
		...
		command.getRequirementsStream().forEach(this::requires);
	 ...
	 * @throws IllegalUseOfCommandException
	 *             if the group has been started before or been given to another
	 *             group or if the {@link Command Command} has been started
	 *             before or been given to another group
	 * @throws IllegalArgumentException
	...
	/**
	 * Adds a new child {@link Command} to the group. The {@link Command} will
	 ...
	 * that the given {@link Command Command} will have no knowledge that it is
	 * on a timer.
	 * </p>
	 *
	 * <p>
	 ...
	 * child with conflicting requirements is started, or the main sequence runs
	 * a {@link Command} with conflicting requirements. In the latter two cases,
	...
	/**
	 * Returns whether or not this group is interruptible. A command group will
	 * be uninterruptible if {@link CommandGroup#setInterruptible(boolean)
	...
	private void cancelConflicts(Command command) {
		...
		m_children.stream().map(Entry::getCommand).filter(c -> {
			return c.getRequirementsStream().anyMatch(command::doesRequire);
		}).forEach(child -> {
			// Cancel this child if it conflicts with any of the newly
			// introduced command's requirements
			child._cancel();
			child.removed();
		});
	}
